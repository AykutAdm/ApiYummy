...
using ApiProje.WebUI.Dtos.WhyChooseYummyDtos;
...
namespace ApiProje.WebUI.Controllers
{
    public class WhyChooseYummyController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7194/api/Services?id=" + id);
            return RedirectToAction("WhyChooseYummyList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7194/api/Services/GetService?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateWhyChooseYummy(UpdateWhyChooseYummyDto updateWhyChooseYummy)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7194/api/Services/", stringContent);
            return RedirectToAction("WhyChooseYummyList");
        }
    }
}
